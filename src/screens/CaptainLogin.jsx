      ...
      <div className="auth-content">
        <div>
          ...
          <div className="glass-card p-4 min-[380px]:p-5">
            <Heading title="Drive with QuickRide" eyebrow="Captain login" />
            <form onSubmit={handleSubmit(loginCaptain)}>
              ...
              <Link to="/captain/forgot-password" className="mb-4 inline-block text-sm font-bold text-slate-500">Forgot password?</Link>
              <Button title="Login" loading={loading} type="submit" />
            </form>
            <p className="mt-5 text-center text-sm font-semibold text-slate-500">Don&apos;t have an account? <Link to="/captain/signup" className="font-black text-slate-950">Sign up</Link></p>
          </div>
        </div>

        <div className="glass-card mt-5 p-4">
          <div className="mb-3 flex items-center gap-3 text-sm font-semibold text-slate-600"><ShieldCheck size={18} /> Secure captain access</div>
          <Button type="link" path="/login" title="Login as Passenger" variant="secondary" />
        </div>
      </div>
    </div>
  );
