...
import { Banknote, ChevronDown, Clock3, PhoneCall, SendHorizontal, Share2, ShieldCheck, Sparkles, Star, Tag } from "lucide-react";
...
<<<<<<< HEAD
        ...
        {confirmedRideData?._id && (
          <div className="mb-4 overflow-hidden rounded-[28px] border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-4 shadow-[0_12px_30px_rgba(16,185,129,.10)]">
            ...
            {(status === "arrived" || status === "accepted" || status === "arriving") && <div className="mt-4 grid grid-cols-[1fr_auto] gap-2">
              ...
              <div className="grid grid-cols-3 gap-2"><Button type="link" path={`/user/chat/${confirmedRideData?._id}`} title="Chat" icon={<SendHorizontal size={17} />} variant="secondary" classes="min-h-[52px] w-auto px-3 py-3 text-xs" /><button type="button" className="icon-btn h-[52px] w-[52px]" onClick={callDriver}><PhoneCall size={18} /></button><button type="button" className="icon-btn h-[52px] w-[52px]" onClick={shareTrip}><Share2 size={18} /></button></div>
            </div>}
          </div>
        )}
        ...
        <div className="mt-3 flex items-center justify-between rounded-[24px] border border-slate-200 bg-white p-4 shadow-[0_10px_28px_rgba(15,23,42,.05)]">
          <div><p className="mini-label">{promoResult?.discount ? "Discounted fare" : "Estimated fare"}</p><h3 className="mt-1 text-2xl font-black tracking-tight text-slate-950">{rideFare}</h3>{promoResult?.discount ? <p className="mt-1 text-[11px] font-black text-emerald-700">Saved {formatMoney(promoResult.discount, currency)} with {promoResult.code}</p> : <p className="mt-1 text-[11px] font-semibold text-slate-500">Cash is paid directly to your driver.</p>}{(routeInfo?.distanceText || routeInfo?.durationText) ? <p className="mt-1 text-[10px] font-black uppercase tracking-wide text-slate-400">{[routeInfo.distanceText, routeInfo.durationText].filter(Boolean).join(" • ")}</p> : null}</div>
          <div className="flex h-12 w-12 items-center justify-center rounded-[18px] bg-emerald-50 text-emerald-700"><Banknote size={20} /></div>
        </div>
        ...
        <div className="mt-5">{rideCreated || confirmedRideData ? <Button title="Cancel ride" loading={loading} variant="danger" fun={() => setShowCancelModal(true)} /> : <Button title={`Confirm • ${rideFare}`} fun={createRide} loading={loading} />}</div>
      </div>
